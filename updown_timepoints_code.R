...
#define groups and timepoints. do not use dashes (underscores are okay)
timepoints <- c("1", "2")
...
updown <- function(input){
      ...
      data_done$timepoint <- factor(data_done$timepoint, levels = timepoints)
...
makegraphs <- function(input){
      ...
      vf_t <- 
            ggplot(data = data_done, aes(x = timepoint, y = vft_tq)) + 
            geom_bar(stat = "summary", fill = "white", size = 0.75, width = .9, position = "dodge", aes(color = group)) +
            geom_dotplot(binaxis = "y", dotsize = 0.4, position = "dodge", aes(fill = group)) +
            stat_summary(geom = "errorbar", fun.data = mean_se, width=.1, size = .5, position = position_dodge(.9), aes(group = group)) +
...
act_time<- ggplot(data = data_done, aes(x = timepoint, y = licking_time)) + 
      geom_bar(stat = "summary", fill = "white", size = 0.75, width = .9, position = "dodge", aes(color = group)) +
      geom_dotplot(binaxis = "y", dotsize = 0.4, position = "dodge", aes(fill = group)) +
      stat_summary(geom = "errorbar", fun.data = mean_se, width=.1, size = .5, position = position_dodge(.9), aes(group = group)) +
      scale_fill_manual(values=c("deepskyblue", "brown1")) +
      ...
      labs(
            x = "Group",
            y = "Licking time (seconds out of 90s)",
            title = "Acetone test over 4 weeks after formalin injection",
            caption = "Two way anova, Tukey post hoc") +
      geom_line(data = ggot_p_time, aes(x = x, y = y, group = 1)) +
      annotate("text", x = 6, y = 9.5, 
               label = "*",
               size = 6, color = "#22292F"
      ) 
...
#code in progress
#model <- lme(vft_tq ~ timepoint * group, random=~1|mouse_nmbr, data=data_done)
#act_anova <- anova(model)
#pmodel <- tibble(act_anova)[3,4]
